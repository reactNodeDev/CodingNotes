...
const decideWinner = ((pandaAvgScore1, koalaAvgScore1, season) => {
    if (pandaAvgScore1 >= 2 * koalaAvgScore1) {
        console.log(`Pandas won season ${season}, their average is ${pandaAvgScore1 / koalaAvgScore1} times more than koalas`);
    } else if (koalaAvgScore1 >= 2 * pandaAvgScore1) {
        console.log(`Koalas won season ${season}, their average is ${koalaAvgScore1 / pandaAvgScore1} times more than Pandas`);
    } else {
...
// decideWinner(pandaAvgScore2,koalaAvgScore2,2);
  ...
  // ---------------------------------------- Array methods -------------------------------------------- // 
let mobiles = ['poco x3 pro', 'iphone 11 pro max', 'samsung galaxy s20 fe', 'oneplus 9r'];
...
const myMobile = {
    ...
    display: '6.4 inch',
    launchYear: 2017,
    calcPrice: function () { // a function inside an object is called a "METHOD"
    ...
    getSummary: function() {
        this.summary = `${this.firstName} ${this.lastName} has a ${this.processor} processor, ${this.display} display, ${this.battery}mah battery. It is priced at ${this.calcPrice()}`;
        return this.summary;
    }
}
...
const tushar = {
    firstName: 'tushar',
    birthYear: 1996,
    calcAge: function(){ //"calcAge" method is defined in "tushar"
...
const anObject = {
    firstName: 'shankar',
    class: '12th',
    getFirstName: () => { //an arrow function
...
const pizzaArr = ['dominos','pizza hut','pizza uncle', 'jungle hut', 'valencia galleria'];
...
const foodMenu = {
    ...
    starterMenu: ['paneer tikka','mushroom tikka','cheese fries','masala kabab'],
...
const dominar400 = {
    ...
    power: '39ps',
    torque: '35nm',
    features: ['dual-channel ABS','USD','clip-on handle bars','slipper-clutch','secondary display'],
...
const {brand:bikeBrand,model:bikeModel,displacement:bikeDisplacement,power:bikePower,torque:bikeTorque} = dominar400;
...
const {sun: {open:oSun,close:cSun}, mon: {open:oMon, close:cMon}, tue: {open:oTue, close:cTue} } = schedule;
...
const swiggy = {
    ...
    orderDetail: function({starterIndex=1,starterAmount=1, mainIndex=1, mainAmount=1, address='India', time="10:10 pm"}){
...
const pizzaPlace = {
    decideIngredients: function(ingredient1, ...others){ //atleast one ingredient is compulsary. rest others are optional//
...
for(let num of numberArr){  //loops the entire array and gives us the Values
    // console.log(num);
}

for(let [i,val] of numberArr.entries()){  // gives us the Values and their Index Number
    // console.log(`${i+1}: ${val}`);
}
...
for(const day of Object.keys(timeTable)){ //this will loop over the keys of "timeTable" obj
    daysStr += `${day},`
}
...
const customerCheck = function(items){
    ...
    let ourResponse;
    if(itemsLowerCase.includes('knife') || itemsLowerCase.includes('gun')) ourResponse = 'you are not allowed !';
...
const censorDebitCardNumber = function(cardNum){
    cardNum = cardNum + '';
    if(cardNum.length !== 16) { console.log("please enter a 16 digit card number");}
    else { 
    const cardNumFirstTwoDigits = cardNum.slice(0,2).padEnd(cardNum.length-2, '*');
...
// ----------------------------------- CLOSER LOOK AT "FUNCTIONS" -------------------------------------------- //
                    ...
                    // ---------- Default Parameters ------------ //
function flightBooking(flightNum = 'AJ129', numOfPassengers = 1, ticketPrice = numOfPassengers*2000) {
    const booking = {
        flightNum,
        numOfPassengers, 
        ticketPrice,
...
function greet(greeting) {
    return function greetUser(name){ //this "return function" will be the result of "greet" function's call
...
const sayHello = greetUser => userName => { //one "arrow function" returning another "arrow function"
...
const airIndia = {
    ...
    book(flightNum, passengerName) {
        ...
        this.bookings.push({PassengerName: `${passengerName}`, flightNum: `${this.iataCode}${flightNum}`});
    },
}
...
const randomArrJoined = randomArr.join('');
        ...
        // ------------------------- forEach -------------------------- //
const movements = [100, 1200, -500, 2000, 3000, -2500, -1000, 4000, 5000, -4500, 2000, -1000];
let totalBalance = 0;
...
const games = new Set(['Total Overdose','Freedom Fighter','FIFA 2007','Smackdown vs Raw','Max Payne 2']);
const gamesallValues =  games.values();

games.forEach(function(elem, _, set){ //as sets dont have "keys", keys argument has '_'
})
...
const rupeesIntoUSD = rupees.map(function(rupee){ //"map" will give us a new array from the "rupees" array
...
const account1 = {
    ...
    pin: 2109,
    transactions: [8000,19000,-7000,3200,-11000,34000,-14000,18750,-11990,+2100],
...
const account2 = {
    ...
    pin: 1092,
    transactions: [1500, 3500, 7900, -5000, -2000, 14550, -1100, -1190, 24900, -14500],
    interestRate: 8.6,
};
...
const account3 = {
    owner: 'Taarak Mehta',
    pin: 1181,
    transactions: [1200, 4500, 900, -5000, -2000, 18550, -11000, -1190, 30000, -14500],
    interestRate: 8.6,
};
...
const createAndAddUsername = function(accs){
    accs.forEach(function(acc){ // 'forEach' method always returns something
...
let bankTransactions = [200, 1000, 5600, -1400, -500, 560, -2000, 10000, -5300, 4000, -2500];
...
bankTransactions = [200, 1000, 5600, -1400, -500, 560, -2000, 10000, -5300, 4000, -2500];

const balance = bankTransactions.reduce(function(acc,trans,i,arr){ // 'acc' - accumulator
    // console.log(`${i+1}: ${acc}`);
    return acc+trans;
...
const biggestTransaction = (transArray) => {
     const biggestNumber = transArray.reduce(function (accumulator,currentElement){
         if(accumulator>currentElement){ //'accumulator' starts from 'array[0]' iterating the array. as long as 'accumulaor > currentElement' keeps happening during the iteration, 'accumulator' keeps returning
             return accumulator;
         } else { //if and when 'accumulator < currentElement' happens during the iteration, 'currentElement' from the 'else' is returned. now, when the iteration continues, returned 'currentElement' from 'else' condition becomes the new 'accumulator' as 'reduce' returns just one value and the 'if' condition starts executing again and this just like this the whole array is iterated
...
const calcAverageHumanAge = function(dogAgeArr) {
 ...
 const humanAge = dogAgeArr.map((age,i,ageArr)=> { //a new array will be returned acc. to the conditions used
        if(age<=2) return 2*age;
        else return 16 + age * 4
    })

 const adultDogs = humanAge.filter((age,i,ageArr) => { // returns a new filtered array acc. to coondition used
...
const tweakedHighScores = highScores.map(score => score+200)
...
highScores.sort((a,b)=> { // 'a' & 'b' represent all the two consecutive elements of the array, where 'a' appears before 'b' in the array
...
let transactionsAcc = [8000,19000,-7000,3200,-11000,34000,-14000,18750,-11990,2100];
...
const allTransactionItems = Array.from(document.querySelectorAll('.transaction_item'), trans => Number(trans.textContent.replace('Rs ','').replace(' /-',''))); // "document.querySelectorAll('.transaction_item')" gives us a node-list array. to make it a normal array, we use "Array.from()". so, 'allTransactionItems' is a proper array. in the second argument, we have used array methods on the array we have got

const sumOfAllTransactionItems = allTransactionItems.reduce((el,con) => con+el,0);

const totalSumElement = document.querySelector('.transaction-sum');
totalSumElement.insertAdjacentHTML('afterbegin',`Total: ${sumOfAllTransactionItems}`);


// ------------------------------- ARRAY METHODS PRACTICE ---------------------------------------------- //

let myTransactions = [8000,19000,-7000,3200,-11000,34000,-14000,18750,-11990,2100, 12500, 19600, 31590, -19000];

const totalTransactionsAbove10K = myTransactions.filter(trans => trans>10000)
...
const {depositsSum,withdrawlsSum} = myTransactions.reduce((acc,trans)=>{
    trans>0 ? acc.depositsSum += trans : acc.withdrawlsSum += trans;
    return acc;
}, {depositsSum:0,withdrawlsSum:0}); //here 'accumulator (acc)' is an object with properties 'depositsSum' & 'withdrawlsSum'. we can make changes to both properties and then return the final 'accumulator(acc)'

// -------------method 2 (using " obj['property'] " method of object destructuring)
const {sumOfDeposits, sumOfWithdrawls} = myTransactions.reduce((acc,trans) => {
...
const capitaliseSentence = function(title) {
    const exceptions = ['am','of','a','the','and','and','or','with','as','but','for', 'if', 'nor','or','so','yet','on','is'];
    const capitaliseString = str =>  str[0].toUpperCase() + str.slice(1);

    const capitalisedTitle = title.split(' ').map(word => word.toLowerCase())
...
const analyseFoodIntake = (arrDog,owner) => {
    ...
    const ownersName = ownersDog?.owners.length>1 ? ownersDog?.owners.join(' and ') : ownersDog?.owners[0]
...
const IsAnyExactRecommendedEatingDog = (arrDog) => arrDog.some(dog => dog.curFood === dog.recommendedFood);

const ownersOfdogsEatingExtra = dogsArr.filter(dog => dog.curFood > 1.1*dog.recommendedFood)
                                       .map(dog => dog.owners).flat();


const ownersOfdogsEatingLess = dogsArr.filter(dog => dog.curFood < 0.9*dog.recommendedFood)
                                      .map(dog => dog.owners).flat();

const overeatingDogsVerdict = (ownersArr,howMuch) => `${ownersArr.join(' and ')}'s dogs eat too ${howMuch}`;

overeatingDogsVerdict(ownersOfdogsEatingExtra,'much')
overeatingDogsVerdict(ownersOfdogsEatingLess,'less')

const dogArrSortedByRecFood = dogArr => dogArr.slice().sort((a,b) => a.recommendedFood-b.recommendedFood);
...
const pizzaTimer =  setTimeout((ing1,_,ing3) => {
...
addNewPlaneBtn.style.height = parseFloat(getComputedStyle(addNewPlaneBtn).height) + 20 + 'px'
...
tabsContainer.addEventListener('click', function(e){
    ...
    const infopara = document.querySelector(`.info-para-${clickedButton.dataset.btn}`); //selecting the 'tabcontent' paragraph based on the 'data-btn' attribute of the buttons
...
class FriendsSitcom {
    constructor(authorFullName, genre, yearBegin, episodeLength, favChar1, favChar2, charNames){
    ...
    set authorFullName (name) {
        if(name.includes(' ')) this._authorFullName = name; // new property '_authorFullName' defined bcz 'authorFullName' already exists in the constructor, so 'set' and 'constructor' will try to set authorFullName together which will give us a max call stack error
...
const friendsInfo = new FriendsSitcom('Tushar Ghildiyal','comedy',1994,21,'Chandler','Ross');
friendsInfo.showAge;
friendsInfo.characterNames = ['Chandler','Monica','Joey','Rachel','Ross','Phoebe'];
...
const bikeProto = {
    init(brandName,modelName,engineCapacity) { //we can name this method anything. 'init' is conventional
...
volleyballPlayer.prototype.introduction = function() {
    return `hi, my name is ${this.firstName}. i am ${2021-this.birthYear} years old. i am a ${this.level} ${this.sport} player`
}
...
ajaxRequest.addEventListener('load', function(){
    ...
    const currency = data.currencies[0].name === 'United States dollar' ? 'USD' : data.currencies[0].name;
    
    const card = `<div class="col-12 col-md-4 mb-3">
    <div class="card bg-dark mx-auto" style="width: 18rem;">
    <img src="${data.flag}" class="card-img-top" style="height:10rem; width:100%" />
    <div class="card-body text-white small-text">
      <p class="card-title mb-0">${data.name}</p>
      <p class="text-info">${data.region}</p>
      <p class="card-text">Capital: <span class="fw-bold" style="color:red">${data.capital}</span></p>
    </div>
    <ul class="list-group list-group-flush bg-dark text-white">
    <li class="list-group-item bg-dark text-white small-text">Language:<span class="fw-bold "> ${data.languages[0].name}</span></li>
      <li class="list-group-item bg-dark text-white small-text">Currency: <span class="fw-bold">${currency}(${data.currencies[0].symbol})</span></li>
      <li class="list-group-item bg-dark text-white small-text">Population: <span class="fw-bold">${(data.population/1000000).toFixed()} Million</span></li>
    </ul>
  </div>
  </div>`;
...
const renderCountry = function(nation) {
    const currency = nation.currencies[0].name === 'United States dollar' ? 'USD' : nation.currencies[0].name;
    ...
    <div class="card-body text-white small-text">
      ...
      <p class="card-text">Capital: <span class="text-danger ">${nation.capital}</span></p>
    </div>
    <ul class="list-group list-group-flush bg-dark text-white">
    <li class="list-group-item bg-dark text-white small-text">Language:<span class="" style="color: #20c997"> ${nation.languages[0].name}</span></li>
      <li class="list-group-item bg-dark text-white small-text">Currency: <span class="" style="color: #20c997">${currency} (${nation.currencies[0].symbol})</span></li>
      <li class="list-group-item bg-dark text-white small-text">Population: <span class="" style="color: #20c997">${(nation.population / 1000000).toFixed()} Million</span></li>
    </ul>
  </div>
  </div>`;
...
const getCountryInfo = function(country) {
    ...
    .then(response => { //this 'then' is for if the promise is rejected
        if(!response.ok) //if country not found from the entered string. if '!res.ok' is true, the promise returned from this 'then' will be rejected with the following error
    ...
    .then(data => { // this 'then' is for if the promise is resolved
        renderCountry(data[0])
        const neighbour = data[0].borders[0];

        if(!neighbour)  throw new Error(`No Neighbour Country found : ${response.status}`) // the 'error' we throw here is caught in the 'catch' block below
    ...
    .then(data => data)
    .catch(err => { // we have declared a callback function for the errors (promise rejection). so, there will be no 'uncaught error' displayed in the console as we have caught those errors in this callback functn
        console.error(err);
...
const fetchCountryInfo = async function(country) {
    ...
    if (!res.ok) //if country not found from the entered string. 
    ...
    const neighbourCountryRes = await fetch(`https://restcountries.eu/rest/v2/alpha/${neighbour}`); //returning the fetching of first neighbour country of the country as a promise
...
const loadCountryPopulation = async function(c1,c2,c3) {
    try {
        ...
        await Promise.all([getJSON(`https://restcountries.eu/rest/v2/name/${c1}`),
